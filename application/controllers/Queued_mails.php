...
class Queued_mails extends CI_Controller {
...
$this->load->model('Queued_mailsModel');
	...
	public function index(){
		...
		$result['Received_messages_new'] = $this->Received_messages_newModel->receivedmsglog();
				$result['queued_data'] = $this->Queued_mailsModel->getdata();

			$this->load->view('queued_mails',$result);
		
	}

	public function getid($id){
		$result = $this->Queued_mailsModel->getid($id);
		echo json_encode($result);
	}
	public function delete($id){
		$result = $this->Queued_mailsModel->delete($id);
		echo $result;
		redirect(base_url().'Queued_Mails');
	}
